...
namespace InternetShop.Server.Semaphore
{
    ...
    {
        private static readonly SemaphoreSlim _stockSemaphore = new SemaphoreSlim(1, 1);
        ...
        public Response CreateOrder(object data, int clientId = 0)
        {
            try
            {
                ...
                _stockSemaphore.Wait();

                try
                {
                    Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] Client {clientId} → SEMAPHORE ENTER");
                    ...
                    foreach (var item in order.Items)
                    {
                        ...
                        if (product == null)
                        {
                            Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] Client {clientId} → SEMAPHORE EXIT (товар не найден)");
                            return new Response
                            {
                                Success = false,
                        ...
                        {
                            Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] Client {clientId} → FAIL");
                            Console.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] Client {clientId} → SEMAPHORE EXIT");
                            return new Response
                            {
                                Success = false,
